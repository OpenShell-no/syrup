...
def findTool(exe, name=None):
    ...
    EXTENSIONS = [
        e.lower() for e in os.environ.get("PATHEXT", ".sh").split(os.pathsep)
    ] + [""]
    ...
    print(
        "WARNING: Unable to find {} ({})".format(exe, name)
...
def p7zip_list(fn):
    ...
    for fileinfo in filelist:
        if not fileinfo.strip():
            continue

        fi = dict(
            [[y.strip() for y in x.split("=", 1)] for x in fileinfo.strip().split("\n")]
        )

        if fi.get("Attributes") == "D":
            continue  # Directory
...
def p7zip_open_file(fn, name):
    TOOLS_7ZIP = findTool("7z") or findTool("7za") or findTool("7zr")
    p = subprocess.Popen(
        [TOOLS_7ZIP, "e", "-so", fn], stdout=subprocess.PIPE, stderr=subprocess.DEVNULL
    )
    return p.stdout
...
def cleanBuild(builddir):
    ...
    if os.path.exists(builddir):
        ...
        shutil.rmtree(
            builddir,
            onerror=lambda func, path, exec_info: print(
                "WARNING: Failed to delete ", path, exec_info
            ),
        )
    ...
    **kwargs,
    ...
    template_variables = {
        ...
        "outfile": os.path.join(
            artifact_dir,
            "${APPNAME}-${VERSIONMAJOR}.${VERSIONMINOR}.${VERSIONBUILD}-setup.exe",
        ),
